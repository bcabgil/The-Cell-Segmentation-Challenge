...
import numpy as np
def u_net_lstm(Base,img_height, img_width, img_ch, batchNormalization, SDRate, spatial_dropout, number_of_labels, activation_function,weighted):
    
    inputs = Input((img_height, img_width, img_ch))
     
    #if the weighted version is selected add another input layer                
    if weighted:
    ...
    a18 = Activation(activation_function)(c22)
                    
    #Check if the weighted model is selected
    if weighted:
